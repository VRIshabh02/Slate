...
import 'package:untitled_slate/Models/tallySyncModel.dart';
...
class _TallySyncState extends State<TallySync> {
  ...
  List<DataTallySync> _list = [];
  List<DataTallySync> _filteredList = [];
            ...
            builder: (context, snapshot) {
              if (snapshot.connectionState ==
                ...
                return FutureBuilder(
                    future: getTallySync(organizationsData.ret.data[company].orgId),
                    builder: (context, snapshot) {
                      if (snapshot.connectionState == ConnectionState.done) {
                        TallySyncModel tallyListData =
                        snapshot.data as TallySyncModel;
                        _list = tallyListData.ret!.data!;
...
// initialValue: "${widget.email}",
                                                        ...
                                                        onChanged: (text) {
                                                          ...
                                                          _list.forEach((userDetail) {
                                                            if ((userDetail.fileName.toString())
                                                                .contains(
                                                                searchController
                                                                    .text) ||
                                                                userDetail
...
// onSearchTextChanged(setWidgetState),
  ...
  onSearchTextChanged(Function setWidgetState) async {
    ...
    _list.forEach((userDetail) {
      if ((userDetail.fileName.toString()).contains(searchController.text) ||
          userDetail.tallyOrgName!.contains(searchController.text))
  ...
  List<Widget> listChildren(List<DataTallySync> list) {
...
Future<TallySyncModel> getTallySync(int orgId) async {
  ...
  if (response.statusCode == 200) {
    return TallySyncModel.fromJson(data);
  } else {
    return TallySyncModel.fromJson(data);
  }
}
